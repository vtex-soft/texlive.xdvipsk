...
syscopyfile(char *s)
{
	...
	if (inhandle == NULL) {
		(void)sprintf(errbuf, "!Couldn't open %s for system I/O!", s);
		error(errbuf);
	}
	else {
		counter = 0;
		...
		if (buff) {
			...
			while ((inbytes = fread(buff, 1, BUFFSIZE, inhandle)) > 0){
				...
				if (inbytes != outbytes) {
					(void)sprintf(errbuf, "!Error writing file %s with system I/O!", s);
					error(errbuf);
					break;
				}
				if (!quiet)
...
/*
 *   Is there room in the buffer?  LINELENGTH-6 is used because we
 *   need room for (, ), and a possible 2 four-byte strings \000, for
 *   instance.  If it is too long, we send out the string.
 */
   jflag = 0;
   if (instring > LINELENGTH-10) {
      stringend();
      chrcmd('p');
   }
#if defined(__BIG_ENDIAN__)
   a = (unsigned char)(c & 0xFF00);
   b = (unsigned char)(c << 8);
#else
   a = (unsigned char)(c >> 8);
