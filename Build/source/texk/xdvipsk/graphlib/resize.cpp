...
/// Clamp function
static int CLAMP(const int &value, const int &min_value, const int &max_value) {
...
GetRGBAPalette(FIBITMAP *dib, RGBQUAD * const buffer) {
	...
	const unsigned ntransp = MIN(ncolors, Bitmap_GetTransparencyCount(dib));
...
FIBITMAP* CResizeEngine::scale(FIBITMAP *src, unsigned dst_width, unsigned dst_height, unsigned src_left, unsigned src_top, unsigned src_width, unsigned src_height, unsigned flags) {
	...
	unsigned src_offset_y = Bitmap_GetHeight(src) - src_height - src_top;
	...
	if (dst_width <= src_width) {
		...
		if (src_width != dst_width) {
			...
			if (src_height != dst_height) {
				...
				// a temporary image
				tmp = Bitmap_AllocateT(image_type, dst_width, src_height, dst_bpp_s1, 0, 0, 0);
				if (!tmp) {
					Bitmap_Unload(dst);
					return NULL;
				}
		...
		if (src_height != dst_height) {
			...
			if (src_width != dst_width) {
				...
				// a temporary image
				tmp = Bitmap_AllocateT(image_type, src_width, dst_height, dst_bpp_s1, 0, 0, 0);
				if (!tmp) {
					Bitmap_Unload(dst);
					return NULL;
				}
			...
			switch (Bitmap_GetBPP(src)) {
						...
						case 8:
						{
							...
							if (src_pal) {
								...
								for (unsigned y = 0; y < height; y++) {
									// scale each row
									const BYTE * const src_bits = Bitmap_GetScanLine(src, y + src_offset_y) + src_offset_x;
									BYTE * const dst_bits = Bitmap_GetScanLine(dst, y);
									...
									for (unsigned x = 0; x < dst_width; x++) {
										...
										dst_bits[x] = (BYTE)CLAMP((int)(value + 0.5), 0, 0xFF);
									}
								}
							} else {
							...
							if (src_pal) {
								...
								for (unsigned y = 0; y < height; y++) {
									...
									for (unsigned x = 0; x < dst_width; x++) {
										...
										// clamp and place result in destination pixel
										dst_bits[FI_RGBA_RED]	= (BYTE)CLAMP((int)(r + 0.5), 0, 0xFF);
										dst_bits[FI_RGBA_GREEN]	= (BYTE)CLAMP((int)(g + 0.5), 0, 0xFF);
										dst_bits[FI_RGBA_BLUE]	= (BYTE)CLAMP((int)(b + 0.5), 0, 0xFF);
										dst_bits += 3;
									}
								}
							} else {
								...
								for (unsigned y = 0; y < height; y++) {
									...
									for (unsigned x = 0; x < dst_width; x++) {
										...
										// clamp and place result in destination pixel
										const BYTE bval = (BYTE)CLAMP((int)(value + 0.5), 0, 0xFF);
						...
						case 32:
						{
							...
							for (unsigned y = 0; y < height; y++) {
								...
								for (unsigned x = 0; x < dst_width; x++) {
									...
									dst_bits[FI_RGBA_GREEN]	= (BYTE)CLAMP((int)(g + 0.5), 0, 0xFF);
									dst_bits[FI_RGBA_BLUE]	= (BYTE)CLAMP((int)(b + 0.5), 0, 0xFF);
									dst_bits[FI_RGBA_ALPHA]	= (BYTE)CLAMP((int)(a + 0.5), 0, 0xFF);
									dst_bits += 4;
								}
							}
						}
				...
				case 16:
				{
					...
					if (IS_FORMAT_RGB565(src)) {
						...
						for (unsigned y = 0; y < height; y++) {
							// scale each row
							const WORD * const src_bits = (WORD *)Bitmap_GetScanLine(src, y + src_offset_y) + src_offset_x / sizeof(WORD);
							BYTE *dst_bits = Bitmap_GetScanLine(dst, y);
							...
							for (unsigned x = 0; x < dst_width; x++) {
								...
								// clamp and place result in destination pixel
								dst_bits[FI_RGBA_RED]	= (BYTE)CLAMP((int)(((r * 0xFF) / 0x1F) + 0.5), 0, 0xFF);
								dst_bits[FI_RGBA_GREEN]	= (BYTE)CLAMP((int)(((g * 0xFF) / 0x3F) + 0.5), 0, 0xFF);
								dst_bits[FI_RGBA_BLUE]	= (BYTE)CLAMP((int)(((b * 0xFF) / 0x1F) + 0.5), 0, 0xFF);
								dst_bits += 3;
							}
						}
					} else {
						...
						for (unsigned y = 0; y < height; y++) {
							// scale each row
							const WORD * const src_bits = (WORD *)Bitmap_GetScanLine(src, y + src_offset_y) + src_offset_x;
							BYTE *dst_bits = Bitmap_GetScanLine(dst, y);
							...
							for (unsigned x = 0; x < dst_width; x++) {
								...
								// clamp and place result in destination pixel
								dst_bits[FI_RGBA_RED]	= (BYTE)CLAMP((int)(((r * 0xFF) / 0x1F) + 0.5), 0, 0xFF);
								dst_bits[FI_RGBA_GREEN]	= (BYTE)CLAMP((int)(((g * 0xFF) / 0x1F) + 0.5), 0, 0xFF);
								dst_bits[FI_RGBA_BLUE]	= (BYTE)CLAMP((int)(((b * 0xFF) / 0x1F) + 0.5), 0, 0xFF);
								dst_bits += 3;
							}
						}
				...
				case 24:
				{
					...
					for (unsigned y = 0; y < height; y++) {
						// scale each row
						const BYTE * const src_bits = Bitmap_GetScanLine(src, y + src_offset_y) + src_offset_x * 3;
						BYTE *dst_bits = Bitmap_GetScanLine(dst, y);
				...
				case 32:
				{
					...
					for (unsigned y = 0; y < height; y++) {
						// scale each row
						const BYTE * const src_bits = Bitmap_GetScanLine(src, y + src_offset_y) + src_offset_x * 4;
						BYTE *dst_bits = Bitmap_GetScanLine(dst, y);
...
void CResizeEngine::verticalFilter(FIBITMAP *const src, unsigned width, unsigned src_height, unsigned src_offset_x, unsigned src_offset_y, const RGBQUAD *const src_pal, FIBITMAP *const dst, unsigned dst_height) {
	...
	switch(Bitmap_GetImageType(src)) {
		case FIT_BITMAP:
		{
			const unsigned dst_pitch = Bitmap_GetPitch(dst);
			BYTE * const dst_base = Bitmap_GetBits(dst);

			switch(Bitmap_GetBPP(src)) {
				case 1:
				{
					const unsigned src_pitch = Bitmap_GetPitch(src);
					const BYTE * const src_base = Bitmap_GetBits(src) + src_offset_y * src_pitch + (src_offset_x >> 3);
							...
							if (src_pal) {
								...
								for (unsigned x = 0; x < width; x++) {
									...
									for (unsigned y = 0; y < dst_height; y++) {
										...
										// clamp and place result in destination pixel
										*dst_bits = (BYTE)CLAMP((int)(value + 0.5), 0, 0xFF);
										dst_bits += dst_pitch;
									}
								}
							} else {
				...
				{
					const unsigned src_pitch = Bitmap_GetPitch(src);
					const BYTE *const src_base = Bitmap_GetBits(src) + src_offset_y * src_pitch + (src_offset_x >> 1);
				...
				{
					const unsigned src_pitch = Bitmap_GetPitch(src);
					const BYTE *const src_base = Bitmap_GetBits(src) + src_offset_y * src_pitch + src_offset_x;
				...
				{
					// transparently convert the 16-bit non-transparent image to 24 bpp
					const unsigned src_pitch = Bitmap_GetPitch(src) / sizeof(WORD);
					const WORD *const src_base = (WORD *)Bitmap_GetBits(src) + src_offset_y * src_pitch + src_offset_x;
				...
				case 32:
				{
					// scale the 32-bit transparent image into a 32 bpp destination image
					const unsigned src_pitch = Bitmap_GetPitch(src);
					const BYTE *const src_base = Bitmap_GetBits(src) + src_offset_y * src_pitch + src_offset_x * 4;
