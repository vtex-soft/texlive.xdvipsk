...
#define BANNER \
"This is make2unc 1.1 Copyright 2016 - 2023 VTeX Ltd"
	...
	for (i = 1; i < xref_len; i++)
	{
		...
		fz_try(ctx)
		{
			...
			if (pdf_is_dict(ctx, dict)) {
				pdf_obj *obj = pdf_dict_get(ctx, dict, PDF_NAME(Type));
				if (pdf_name_eq(ctx, obj, PDF_NAME(Font))) {
					obj = pdf_dict_get(ctx, dict, PDF_NAME(Subtype));
					if (pdf_name_eq(ctx, obj, PDF_NAME(Type0))) {
						obj = pdf_dict_get(ctx, dict, PDF_NAME(ToUnicode));
						if (!obj) {
							obj = pdf_dict_get(ctx, dict, PDF_NAME(BaseFont));
							if (obj) {
								std::string fntName = pdf_to_name(ctx, obj);
								if ((fntName.size() > 7) && (fntName.at(6) == '+'))
									fntName = fntName.substr(7);
								size_t pos = fntName.rfind("-TeX_Identity-H");
								if (pos > 0)
								...
								if (!print_cmap(ctx, buf, orig_cmap, registry, ordering, supplement))
									continue;
								pdf_dict_put_drop(ctx, dict, PDF_NAME(ToUnicode), pdf_add_stream(ctx, doc, buf, NULL, 0));
...
int ModifyPdfForUnicodes(const std::string& name, const Options& options) {
	...
	fz_try(ctx)
	{
		...
		pdf_obj *lua = pdf_new_name(ctx, "LuaTexCmaps");
