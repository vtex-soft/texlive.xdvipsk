/* Copyright (c) 2017 Arunas Povilaitis VTeX Ltd. All rights reserved.
	...
	std::string &registry, std::string ordering, std::string supplement)
{
	...
	for (r = 0; r < cmap->rlen; r++) {
		while (mranges && (ranges[r].low > mranges[m].low) && (mranges[m].low > 0)) {
			...
			block.r[block.size].len = mranges[m].len;
			memcpy(block.r[block.size].out, mranges[m].out, mranges[m].len * sizeof(unsigned int));
			block.size++;
			if (m < cmap->mlen)
				m++;
			else
