...
## *xdvipsk*: extended dvips (TeXLive 2017)
...
3. Convert the `PostScript` file to `PDF` (using Ghostscript, Acrobat or any other tool):
...
This information is the form as used in the `luaotfload` package, except the
...
```
    <tex charcode>,<OT font glyph index>,<unicode equivalent>
    ...
    59967,710,0304
    59968,711,02DA
    59969,712,0020030A0301
    59970,713,0020030A0301
    59971,714,030A
    59972,715,02DC
```
...
extra libraries: `libjpeg`, `libtiff` for `xdvisk`, 
and `libmupdf` for new utility `make2unc`.
Other needed libraries are taken from TeX Live distributions.

The current `xdvipsk` version is based on `dvips 5.997`, `web2c + kpathsea 6.2.3`, `TeXLive 2017.06.21`.
